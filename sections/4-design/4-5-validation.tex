...
if and only if $zs$ is the empty list, i.e.
...
Recall that we also forbid the arguments from binding new variables,
or using variables bound in the expression being validated--
as such the input environment to \gens will be exactly the same as the environment output from \gens
(in the formal rule, both appear as \env.)
...
in this case,
the rule checks that $\envget{\lab}$ is indeed $v$.
...
$\forall 0 \leq i < n . (\env_{i}, \pathput{\lab}{i}) ys_i \gennedby r \gennedarrow ys_{i + 1}, \env_{i + 1}$.
